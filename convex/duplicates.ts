...
import { findDuplicateBookings } from "./analysis/duplicates"
...
export const runScan = mutation({
  ...
  handler: async (ctx, args) => {
    const runId = await ctx.db.insert("duplicateRuns", {
      status: "running",
      startedAt: Date.now(),
    })
    ...
    try {
      const lines = await ctx.db
        ...
        .collect()
      const documentCount = new Set(lines.map((line) => line.document_id)).size
      const findings = findDuplicateBookings(lines, args.limit ?? 25)

      for (const finding of findings) {
        await ctx.db.insert("duplicateFindings", {
          runId,
          title: finding.title,
